/**
 * Admin Monitor Page JavaScript
 * Enhanced version of monitor.js with photo, email, and warning email functionality
 */
...
let currentTooltipData = null; // Store current tooltip tool data for email sending
...
/**
 * Fetch tools status from admin API (includes email + photo)
 */
...
function renderTable(tools) {
    ...
    tools.forEach(tool => {
        ...
        // Borrower name (clickable with enhanced tooltip data)
        ...
        if (tool.status === 'borrowed' && tool.borrower_name) {
            ...
            const toolIndex = toolsData.indexOf(tool);
...
function removeStatusFilter(status) {
    const idx = activeStatusFilters.indexOf(status);
    if (idx > -1) activeStatusFilters.splice(idx, 1);
...
// ==================== Enhanced Borrower Tooltip ====================

/**
 * Show enhanced borrower tooltip with photo, email, and warning button
 */
function showBorrowerTooltip(event, element) {
    event.stopPropagation();

    const toolIndex = parseInt(element.getAttribute('data-tool-index'));
    const tool = toolsData[toolIndex];
    if (!tool) return;

    // Hide any existing tooltip first (without nullifying data yet)
    if (currentTooltip) {
        currentTooltip.classList.add('hidden');
        currentTooltip = null;
    }

    // NOW store current tool data for email sending
    currentTooltipData = tool;

    const tooltip = document.getElementById('borrower-tooltip');

    // Set name & NIM
    document.getElementById('tooltip-name').textContent = tool.borrower_name || '-';
    document.getElementById('tooltip-nim').textContent = `NIM: ${tool.borrower_nim || '-'}`;

    // Set email
    const emailEl = document.getElementById('tooltip-email');
    emailEl.textContent = tool.borrower_email && tool.borrower_email !== '-'
        ? `📧 ${tool.borrower_email}` : '📧 Email tidak tersedia';

    // Set photo
    const photoContainer = document.getElementById('tooltip-photo-container');
    if (tool.borrower_photo_url && tool.borrower_photo_url.length > 0) {
        const img = document.createElement('img');
        img.src = tool.borrower_photo_url;
        img.alt = `Foto ${tool.borrower_name}`;
        img.className = 'w-20 h-20 rounded-full object-cover border-2 border-primary-200 bg-primary-50';
        img.onerror = function () {
            photoContainer.innerHTML = getPhotoFallback(tool.borrower_name);
        };
    ...
    const emailBtn = document.getElementById('tooltip-email-btn');
    if (!tool.borrower_email || tool.borrower_email === '-') {
        emailBtn.disabled = true;
        emailBtn.classList.add('opacity-50', 'cursor-not-allowed');
        emailBtn.classList.remove('hover:bg-primary-600', 'active:bg-primary-700');
    } else {
        emailBtn.disabled = false;
        emailBtn.classList.remove('opacity-50', 'cursor-not-allowed');
        emailBtn.classList.add('hover:bg-primary-600', 'active:bg-primary-700');
    }
...
/**
 * Generate photo fallback with initials
 */
function getPhotoFallback(name) {
    const initials = getInitials(name);
    return `
        <div class="w-20 h-20 rounded-full bg-gradient-to-br from-primary-400 to-primary-600 flex items-center justify-center text-white text-xl font-bold border-2 border-primary-200 shadow-inner">
            ${escapeHtml(initials)}
        </div>
    `;
}
...
/**
 * Send warning email to current borrower
 */
function sendWarningEmail() {
    if (!currentTooltipData) return;

    const tool = currentTooltipData;

    if (!tool.borrower_email || tool.borrower_email === '-') {
        showToast('Email mahasiswa tidak tersedia', 'error');
        return;
    }

    // Set button to loading state
    const btn = document.getElementById('tooltip-email-btn');
    const btnText = document.getElementById('tooltip-email-btn-text');
    btn.disabled = true;
    btn.classList.add('opacity-75');
    btnText.textContent = 'Mengirim email...';

    // Prepare payload — send borrow_date as ISO string for backend duration calculation
    const borrowDate = parseBorrowTime(tool.borrow_time);
    const payload = {
        student_name: tool.borrower_name,
        student_email: tool.borrower_email,
        tool_name: tool.name,
        borrow_date: borrowDate ? borrowDate.toISOString() : ''
    };
    ...
    fetch('/api/admin/send_warning_email', {
        ...
        .then(data => {
            if (data.success) {
                // Success state
                btn.classList.remove('bg-primary-500', 'opacity-75');
                btn.classList.add('bg-green-500');
                btnText.textContent = '✅ Email terkirim!';
                showToast('Email warning berhasil dikirim!', 'success');
                ...
                btnText.textContent = '❌ Gagal - Klik untuk coba lagi';
                btn.disabled = false;
...
/**
 * Reset email button to default state
 */
function resetEmailButton() {
    const btn = document.getElementById('tooltip-email-btn');
    const btnText = document.getElementById('tooltip-email-btn-text');
    btn.classList.remove('bg-green-500', 'bg-red-500', 'opacity-75', 'opacity-50', 'cursor-not-allowed');
    btn.classList.add('bg-primary-500', 'hover:bg-primary-600', 'active:bg-primary-700');
    btn.disabled = false;
    btnText.textContent = 'Kirim Email Warning';
    btn.onclick = function (e) { e.stopPropagation(); sendWarningEmail(); };
}
